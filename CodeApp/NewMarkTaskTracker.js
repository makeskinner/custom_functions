...
function transformOpportunities(accountsArray) {
  ...
  const oppRecords = get(account, 'Opportunities.records', []);
    ...
    // --- PHASE 2: CONSOLIDATED EVENT ENGINE ---
    const eventRecords = get(account, 'Events.records', []);
    const now = new Date();
    
    // Thresholds for Management Benchmark logic
    ...
    const formattedEvents = eventRecords.map(event => {
        ...
        if (eventDate) {
            // WORKSHOP LOGIC (Catch all variants per framework)
            ...
            // MEETING LOGIC (Consolidated counting for historical view)
        ...
        return { subject: get(event, 'Subject'), type: eType, date: eDateStr, delivered: eDelivered };
    });
    ...
    // NEW: Calculate Expansion Level using BOTH ARR and Active Users (Tipping Point Check)
    ...
    const agentPayload = {
        ...
        ve: { p: pastMeetingsL60D, d: workshopsDelivered, events: formattedEvents },
    ...
    transformedItems.push({
        ...
        // BLOCK 12: MANAGEMENT & EVENTS (RESTORED & OPTIMIZED)
        calculatedPriority: priorityVal,
        pastMeetingsCount: pastMeetingsL60D,
        pastMeetingsL12M: pastMeetingsL12M,
        upcomingMeetingsCount: upcomingMeetings,
        workshopsDeliveredCount: workshopsDelivered,
        workshopsPlannedCount: workshopsPlanned,
        totalEngagementsLast60Days: (pastMeetingsL60D + workshopsDelivered),
